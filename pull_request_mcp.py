#!/usr/bin/env python3
"""
A MCP server that provides tools for analyzing GitHub pull requests and integrating with Microsoft Teams.
It includes tools to analyze file changes, suggest PR templates, and check CI/CD status.
"""
...
mcp = FastMCP("pull-request-mcp")

# PR template directory (shared between starter and solution)
TEMPLATES_DIR = Path(__file__).parent / ".github" / "templates"

# Event files for GitHub Actions events
...
async def analyze_file_changes(
    ...
    try:
        ...
        if working_directory is None:
            try:
                # If running in Claude Code, we can get the working directory from the roots
...
async def get_recent_actions_events(limit: int = 10) -> str:
    ...
    if not GITHUB_WORKFLOWS_EVENTS_FILE.exists():
        return json.dumps([])
    
    with open(GITHUB_WORKFLOWS_EVENTS_FILE, 'r') as f:
        events = json.load(f)
...
async def format_ci_failure_alert():
    """
    Create a Microsoft Teams alert for CI/CD failures with rich formatting.
    """

    return """Format this GitHub Actions failure as a Microsoft Teams message using ONLY Microsoft Teams markdown syntax:
...
async def format_ci_success_summary():
    """
    Create a Microsoft Teams message celebrating successful deployments.
    """

    return """Format this successful GitHub Actions run as a Microsoft Teams message using ONLY Microsoft Teams markdown syntax:
...
if __name__ == "__main__":
    ...
    print("Starting pull-request-mcp server...")
